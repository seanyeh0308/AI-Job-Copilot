...
export default async function ResultPage({ params }: { params: { id: string } }) {
  ...
  const [
    ...
    supabase.from("analyses").select("*, analysis_results(*)").eq("id", params.id).single()
  ]);
  ...
  const analysis = analysisQuery.data as AnalysisRecord;
    ...
    <div className="space-y-6">
      <section className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <h1 className="mt-3 text-3xl font-bold text-ink">{analysis.job_title || "Untitled role"}</h1>
          <p className="mt-2 text-muted">{analysis.company_name || "Company not specified"}</p>
        </div>
